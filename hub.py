...
def ensure_questions(num_needed):
    try:
        ...
        if count < num_needed:
            st.warning("Not enough questions in the database. Please pre-generate more questions using the script.")
...
def get_random_questions(num_questions):
    try:
        ...
        if rows is None or len(rows) == 0:
            return []
        return random.sample(rows, min(num_questions, len(rows)))
    except Exception as e:
